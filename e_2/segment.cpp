...
bool intersect(const CSegment& first, const CSegment& second) {
    return projection_intersect(first.start.x, first.end.x, second.start.x, second.end.x)
		&& projection_intersect(first.start.y, first.end.y, second.start.y, second.end.y)
